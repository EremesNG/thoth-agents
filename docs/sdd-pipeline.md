...
When the selected persistence mode includes thoth-mem, follow the installed
provider guidance for retrieval, compaction, and recovery. This route keeps
only SDD phase ordering, artifact identity (`sdd/{change}/{artifact}`),
authorization, resumable checkpoint outcomes, and evidence-only capability
reporting; it does not prescribe provider calls or lifecycle sequencing.
