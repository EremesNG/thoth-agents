...
- Define and expose the built-in MCP endpoints (exa, context7, grep.app) alongside the shared type aliases so the application can treat remote and local MCPs uniformly (`src/mcp/index.ts`, `src/mcp/types.ts`).
...
- Each service file exports a config literal that points at the remote URL or local command and optionally supplies headers or environment variables derived from the corresponding source to avoid leaking secrets (`exa.ts`, `context7.ts`, `grep-app.ts`).
