...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'thoth-agents', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[thoth-agents] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  try {
    ...
    console.error(
      `[thoth-agents] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
