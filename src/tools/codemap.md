...
The `src/tools/` directory provides the core tool implementations for the thoth-agents plugin. It exposes two main categories of tools:
...
- **Install location**: `~/.cache/thoth-agents/bin/sg` (Linux/macOS), `%LOCALAPPDATA%\thoth-agents\bin\sg.exe` (Windows)
