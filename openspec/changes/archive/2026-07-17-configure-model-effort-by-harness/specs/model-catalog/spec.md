# Model Catalog Delta Specification

## Assumptions

- `models.dev` exposes model metadata that can be normalized into supported effort options without changing the model catalog's existing model identity semantics.
- A validated last-known-good (LKG) catalog can be persisted independently from HTTP cache metadata.
- Manual catalog entries remain a supported fallback and may coexist with dynamically discovered entries.
...
- Keep fetching, validation, cache metadata, LKG persistence, and manual fallback selection separable for focused tests.
- Validate the complete remote payload before promoting it to LKG; partial normalization MUST NOT corrupt the prior LKG.
- Cover HTTP 304, malformed responses, empty effort metadata, and first-run offline behavior.
