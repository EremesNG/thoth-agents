...
The `src/cli/` directory provides the command-line interface for installing and configuring **thoth-agents**, an OpenCode plugin. It handles:
...
| `thoth-agents.json` | `~/.config/opencode/` | Plugin-specific lite config |
...
InstallConfig (preferences)
         ...
         │    - Plugin registration ('thoth-agents')
