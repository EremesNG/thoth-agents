# Spec: sdd-design-authoring

Optional, complexity-gated plan sub-artifacts alongside the always-present,
authoritative `design.md`. When complexity warrants and the mechanism is enabled,
`sdd-design` MAY also produce `research.md`, `data-model.md`, a `contracts/`
subdirectory, and `quickstart.md` under `openspec/changes/{change-name}/`.
Gated by `rules.design.sub_artifacts` and `rules.design.complexity_threshold`.
Fully back-compatible: absence of the sub-artifacts is today's behavior.
