# Spec: sdd-plan-review-persistence

Persist Oracle plan-review outcomes as durable, recoverable SDD artifacts with deterministic freshness metadata so resumed workflows can safely reuse valid approvals and reject stale or missing evidence.

## Requirements

### Requirement: Canonical Plan-Review Artifact

For persistence modes that include OpenSpec, the SDD pipeline MUST persist each Oracle `plan-review` result as the canonical artifact `openspec/changes/{change-name}/plan-review.md`. The artifact MUST be human-readable and MUST preserve enough structured data for deterministic recovery, including at minimum: status token, reviewer role, review timestamp, pipeline type, persistence mode, comments, non-blocking notes, blockers, user override context when applicable, and reviewed artifact freshness data.
...
- AND the artifact MUST record the status token, reviewer role, review timestamp, pipeline type, persistence mode, comments, non-blocking notes, blockers, and reviewed artifact freshness data
...
- GIVEN an accelerated pipeline that has proposal and tasks but no full-pipeline spec or design artifacts
- WHEN plan-review is persisted
- THEN the artifact MUST record freshness data for proposal and tasks
- AND it MUST NOT require absent full-pipeline artifacts to exist

### Requirement: Fresh Approval Satisfies Plan-Review Gate

SDD recovery MUST treat a saved fresh `[OKAY]` plan-review artifact as satisfying the `plan-review` gate. Recovery MUST NOT rerun Oracle solely because session state changed. Recovery MUST keep the subsequent user implementation confirmation as a distinct gate that is not satisfied by plan-review approval.

#### Scenario: New session reuses fresh approval

- GIVEN `openspec/changes/{change-name}/plan-review.md` contains `[OKAY]`
- AND reviewed artifact freshness data still matches current planning artifacts
- WHEN the SDD workflow resumes in a new session
- THEN recovery MUST treat the `plan-review` gate as complete
- AND it MUST NOT rerun Oracle solely due to session change
...
SDD recovery MUST treat a saved approval as stale when any reviewed planning artifact has changed since the plan-review artifact was written. A stale approval MUST NOT satisfy the `plan-review` gate and MUST require rerunning Oracle before advancement, unless an explicit user override is captured through the normal blocking-input surface.
...
- GIVEN a full-pipeline `[OKAY]` plan-review artifact exists and records freshness for a reviewed delta spec file
...
- GIVEN a plan-review artifact exists with a status other than `[OKAY]` or equivalent explicit approval token
