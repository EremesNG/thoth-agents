# Delta for sdd-clarification

Introduce a dedicated `sdd-clarify` phase inserted into the Full SDD pipeline
between `spec` and `design`. It performs a taxonomy-driven scan of residual
ambiguity in the authored spec, runs bounded Q&A within the existing
clarification cap, writes resolutions back into the spec, and re-validates the
...
- The clarify phase is Full-SDD only; the accelerated pipeline does not include
  it (accelerated produces no spec).
- Bounded Q&A reuses the existing clarification policy cap
  (`rules.clarification.max_markers_per_spec`, default 3) as the bound on the
  number of clarification questions/resolutions per spec file, aligning with the
  established clarification discipline rather than introducing a new cap.
- The write-back target (in-place spec edit vs. a new artifact) and the
  thoth-mem topic key (`sdd/{change}/spec` vs. a `spec-clarified` key) are
  deferred to the design phase (open question (a)); this spec requires only that
  resolutions be reflected in the authoritative spec content consumed by design,
  without fixing the storage representation.
