...
- `cache.ts` is responsible for invalidating cached installs (`node_modules`, `package.json`) before a fresh `pnpm install`, keeping the cached package state consistent with the server-provided latest version.
...
- When an update is available, the hook either notifies the user or, if `autoUpdate` is on, updates the pinned entry in the OpenCode config (`updatePinnedVersion`), invalidates the cached package, and runs `pnpm install` safely (`runPnpmInstallSafe`) with a 60-second timeout before showing success/error toasts.
...
- `cache.ts` runs before reinstall to remove lingering plugin directories and dependency entries, ensuring `runPnpmInstallSafe` operates on a clean slate.
...
- Uses shared `../../utils/logger` for tracing background operations and errors while talking to platform APIs (`fetch`, package-manager shell calls, `fs`) to inspect, mutate, and reinstall the `thoth-agents` package stored under `CACHE_DIR`.
