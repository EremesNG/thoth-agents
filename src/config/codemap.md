...
- User config: `~/.config/opencode/thoth-agents.jsonc` (preferred) or `.json`
- Project config: `<directory>/.opencode/thoth-agents.jsonc` (preferred) or `.json`
- Environment override: `THOTH_AGENTS_PRESET`
...
- `McpName`: Union type of available MCPs (`'exa'`, `'context7'`, `'grep_app'`)
...
│   └─→ findConfigPath(~/.config/opencode/thoth-agents)
...
│   └─→ findConfigPath(<directory>/.opencode/thoth-agents)
│
...
│   └─→ THOTH_AGENTS_PRESET takes precedence
│
...
│   ├─→ If preset is safe:
│   │   1) ~/.config/opencode/thoth-agents/{preset}
│   │   2) ~/.config/opencode/thoth-agents
│   └─→ Otherwise:
│       1) ~/.config/opencode/thoth-agents
│
