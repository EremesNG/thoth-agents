...
- [ ] 1.1 Add/extend subagent prompt-generation tests for bounded context tools in both read-only and write-capable roles — `src/agents/index.test.ts`, `src/agents/prompt-rendering.test.ts`
  **Verification**:
...
- [ ] 1.2 Add/extend memory-governance model tests for consistent tool matrices and prohibitions — `src/harness/core/memory-governance.test.ts`
  **Verification**:
...
- [ ] 1.3 Add/extend skill packaging/install tests to lock updated thoth-mem-agents wording distribution — `src/harness/writers/skill-layout.test.ts`, `src/cli/custom-skills.test.ts`
  **Verification**:
...
- [ ] 2.1 Update subagent prompt rule generation to explicitly include bounded context-tool usage — `src/agents/prompt-sections.ts`
  **Verification**:
  - Run: `pnpm test -- src/agents/index.test.ts src/agents/prompt-rendering.test.ts`
...
- [ ] 2.2 Update memory-governance model/rules to represent the same tool contract consistently — `src/harness/core/memory-governance.ts`
  **Verification**:
  - Run: `pnpm test -- src/harness/core/memory-governance.test.ts`
...
- [ ] 3.1 Update `thoth-mem-agents` skill wording to keep 3-layer recall canonical while allowing bounded context-tool use — `src/skills/thoth-mem-agents/SKILL.md`
  **Verification**:
  - Run: `pnpm test -- src/harness/writers/skill-layout.test.ts src/cli/custom-skills.test.ts`
...
- [ ] 3.2 Inspect canonical OpenSpec requirements and align updated governance wording only if drift is confirmed — `openspec/specs/multi-harness-agent-pack/spec.md`, `openspec/specs/skill-instructions/spec.md`
  **Verification**:
...
- [ ] 4.1 Run focused regression set for prompts, governance, and skill packaging — targeted files above
  **Verification**:
...
- [ ] 4.2 Run static/type validation for repository consistency — all TypeScript sources
  **Verification**:
  - Run: `pnpm run typecheck && pnpm run lint`
  - Expected: Zero TypeScript errors and zero lint violations.

- [ ] 4.3 Run build and full suite after focused checks pass — repository
  **Verification**:
