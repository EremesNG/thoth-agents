...
- **tmux.ts**: Terminal multiplexer pane lifecycle management—spawning, closing, layout rebalancing, and server health probing for delegated task sessions
...
- **Consumers**: Tmux session manager spawns/closes panes for native task sessions, MCP protocol layer checks for internal initiator markers, polling supports stability checks, ZIP extraction supports plugin updates, agent variant is applied in request pipeline
